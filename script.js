/* this is the javascript file for twitch-fcc */
$(document).ready(function() {
  ...
  },
  {
	...
	/* create the initial list of all users */
	var html = "";
	json.forEach(function(val) {
		if (val.stream === null) {
			html += "<div class='user'>" 
			+ "<a target='_blank' href='https://www.twitch.tv/" + val.display_name + "'><span class='logo-default'></span></a>"
			+ "<div class='display-name'>" + val.display_name + "</div>" 
			+ "</div>";
		} 
		else if (val.stream === undefined ) {
			// do nothing
		}
		else {
			html += "<div class='user'>" 
			+ "<a target='_blank' href='https://www.twitch.tv/" + val.stream.display_name + "'><img  class= 'logo' src='" + val.stream.logo + "'></a>" 
			+ "<div class='display-name'>" + val.stream.display_name + "</div>" 
      + "<div class='current-stream'>Currently Streaming: " + val.stream.status + "</div>" 
			+ "</div>";
		}
  ...
  /* change the display when online menu is clicked */
  
  $("#online").click(function(){
    var online  = "";
    json.forEach(function(val) {
      if (val.stream != null || val.stream != undefined) {
        online += "<div class='user'>" 
        + "<a target='_blank' href='https://www.twitch.tv/" + val.stream.display_name + "'><img  class= 'logo' src='" + val.stream.logo + "'></a>"  
        + "<div class='display-name'>" + val.stream.display_name + "</div>" 
        + "<div class='current-stream'>Currently Streaming: " + val.stream.status + "</div>"
        + "</div>";
    ...
    $(".user-list").html(online);
  });

  /* change the display when all is clicked */
  $("#all").click(function() {
    var all = "";
    json.forEach(function(val) {
      if (val.stream === null) {
  ...
  /* change the display when offline is clicked */
  $("#offline").click(function() {
    var offline = "";
    json.forEach(function(val) {
      if (val.stream === null) {
        offline+= "<div class='user'>" 
        + "<a target='_blank' href='https://www.twitch.tv/" + val.display_name + "'><span class='logo-default'></span></a>"
        + "<div class='display-name'>" + val.display_name + "</div>" 
        + "</div>";
      }
    });
    $(".user-list").html(offline);
  });

  // list search code //
  $(".searchbox").keyup(function(e){
    var value = $(".searchbox").val(); // get the value of the input box.
    // match it with each user's display name in the json and update the user list.
    var results = "";
    json.forEach(function(val) {
      if (val.stream === null) {
        if (val.display_name.toUpperCase().indexOf(value.toUpperCase()) !== -1) {
          results += "<div class='user'>" 
        + "<a target='_blank' href='https://www.twitch.tv/" + val.display_name + "'><span class='logo-default'></span></a>"
        + "<div class='display-name'>" + val.display_name + "</div>" 
        + "</div>";
        ...
        if (val.stream.display_name.toUpperCase().indexOf(value.toUpperCase()) !== -1 ) {
          results += "<div class='user'>" 
        + "<a target='_blank' href='https://www.twitch.tv/" + val.stream.display_name + "'><img  class= 'logo' src='" + val.stream.logo + "'></a>" 
        + "<div class='display-name'>" + val.stream.display_name + "</div>" 
        + "<div class='current-stream'>Currently Streaming: " + val.stream.status + "</div>"
